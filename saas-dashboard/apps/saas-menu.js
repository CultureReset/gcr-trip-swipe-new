...
async function saasGetMenu() {
  const token = getToken();
  const res = await fetch(`${API_BASE}/api/saas/user/module-data/menu`, {
    headers: { 'Authorization': `Bearer ${token}` }
  });
  return res.json();
}

async function saasUpdateMenuItem(id, data) {
  const token = getToken();
  const res = await fetch(`${API_BASE}/api/saas/user/menu/items/${id}`, {
    method: 'PUT',
...
async function saasDeleteMenuItem(id) {
  const token = getToken();
  const res = await fetch(`${API_BASE}/api/saas/user/menu/items/${id}`, {
    method: 'DELETE',
    headers: { 'Authorization': `Bearer ${token}` }
...
async function saasCreateMenuItem(data) {
  const token = getToken();
  const res = await fetch(`${API_BASE}/api/saas/user/menu/items`, {
    method: 'POST',
    headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
    body: JSON.stringify(data)
  });
...
function loadMenu() {
  ...
  saasGetMenu().then(function(items) {
...
function saveCategory() {
  ...
  if (oldName && oldName !== name) {
    ...
    var promises = toRename.map(function(item) {
      return saasUpdateMenuItem(item.id, { category: name });
    });
...
function deleteCategory(name, type) {
  ...
  var promises = toDelete.map(function(item) {
    return saasDeleteMenuItem(item.id);
  });
  Promise.all(promises).then(function() {
...
function saveMenuItem() {
  ...
  if (id) {
    // Update existing
    saasUpdateMenuItem(id, dbPayload).then(function(updated) {
      var item = _menuItems.find(function(i) { return String(i.id) === String(id); });
      if (item) {
        item.name = name;
        item.price = price;
    ...
    // Create new
    saasCreateMenuItem(dbPayload).then(function(created) {
      _menuItems.push({
        id: created.id,
        dbId: created.id,
...
function deleteMenuItem(id) {
  ...
  saasDeleteMenuItem(id).then(function() {
...
async function runMenuExtraction() {
  ...
  var base64, sendMime;
  try {
    var compressed = await CC.compressImage(file, 1280, 0.75);
...
async function importExtractedItems() {
  ...
  for (var ci = 0; ci < _extractedData.categories.length; ci++) {
    ...
    for (var ii = 0; ii < items.length; ii++) {
      ...
      try {
        var created = await saasCreateMenuItem(dbPayload);
        if (created) {
          _menuItems.push({
